...
namespace Wowsome.Anim {
  public static class WAnimFactory {
    public static InterpolationSequence<InterpolationVec2, Vector2> Pulse(float timeMultiplier = 1f, int count = 1, Easing easing = Easing.Linear) {
      List<InterpolationVec2> tws = new List<InterpolationVec2>();

      for (int i = 0; i < count; ++i) {
        tws.Add(new InterpolationVec2(new Timing(.09f * timeMultiplier, easing), new Vector2(1f, 1f), new Vector2(.9f, 1.1f)));
        tws.Add(new InterpolationVec2(new Timing(.08f * timeMultiplier, easing), new Vector2(.9f, 1.1f), new Vector2(1.05f, .95f)));
        tws.Add(new InterpolationVec2(new Timing(.06f * timeMultiplier, easing), new Vector2(1.05f, .95f), new Vector2(.95f, 1.05f)));
        tws.Add(new InterpolationVec2(new Timing(.05f * timeMultiplier, easing), new Vector2(.95f, 1.05f), new Vector2(1f, 1f)));
      }
